...
def client_fn(context: Context):
    ...
    # IID / Non-IID configuration
    # Priority: iid-level (new) > non-iid + dirichlet-alpha (legacy)
    iid_level = context.run_config.get("iid-level", None)
    if iid_level is not None:
        # iid-level takes precedence; load_data will resolve non_iid + alpha internally
        trainloader, valloader = load_data(
            partition_id, num_partitions, hf_dataset,
            total_max_samples=total_max_samples,
            iid_level=float(iid_level),
        )
    else:
        # Legacy fallback: read non-iid and dirichlet-alpha directly
