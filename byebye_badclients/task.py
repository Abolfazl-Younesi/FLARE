...
_IID_LEVEL_MAP: dict = {
    ...
    0.7: (True,  10.0),  # Slightly Non-IID
    0.5: (True,  1.0),   # Moderately Non-IID
    0.3: (True,  0.5),   # Strongly Non-IID
    0.1: (True,  0.1),   # Extremely Non-IID
}


def iid_level_to_params(iid_level: float) -> tuple:
    """Map an iid_level float to (non_iid: bool, dirichlet_alpha: float).

    Supported levels: 1.0 (IID), 0.7, 0.5, 0.3, 0.1 (extremely Non-IID).
    The closest supported level is automatically selected.
    """
...
def load_data(
    ...
    iid_level: float = None,
):
    """Return:
       -> trainloader for this client
       -> global testloader (shared across all clients)

    If *iid_level* is provided it takes precedence over *non_iid* and
    *dirichlet_alpha*.  Supported levels: 1.0, 0.7, 0.5, 0.3, 0.1.
    """
    # Resolve iid_level → non_iid + dirichlet_alpha
    if iid_level is not None:
    ...
    # DataLoaders
    trainloader = DataLoader(train_subset, batch_size=32, shuffle=True, drop_last=False)
    testloader = DataLoader(test_set, batch_size=64, shuffle=False, drop_last=False)
...
def random_update_fn(net: nn.modules.Module):
    ...
    for param in params:
        param.data = torch.randn_like(param.data).to(param.device)
...
def mean_and_sigma_flat(parameters_list):
    ...
    mean_flat = torch.mean(stacked, dim=0).to(device)
    sigma_flat = torch.std(stacked, dim=0, unbiased=False).to(device)
...
def statistical_mimicry_fn(net: torch.nn.Module, cid, alpha=0.5):
    ...
    if loaded_state:
        ...
        state = {
            ...
            "direction": torch.randn_like(flatten_parameters(current_params)).to(net.device if hasattr(net, 'device') else 'cpu'),
    ...
    # Move g_flat to the same device as net parameters
    net_device = next(net.parameters()).device
    g_flat = g_flat.to(net_device)
...
def train(net, trainloader, epochs, device, flip_labels=False, random_update=False, update_scaling=False, alie=False, statistical_mimicry=False, factor=2, num_classes=10, cid=None):
    ...
    avg_trainloss = running_loss / (len(trainloader) * epochs) if (len(trainloader) > 0 and epochs > 0) else 0.0
