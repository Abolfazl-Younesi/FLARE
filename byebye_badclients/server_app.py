...
def process_evaluate_results(results: dict[str, list[float]], out_dir: str):
    base_path = os.path.join(out_dir, "performance")
...
def collect_evaluate_results(metrics_dict: dict[str, float], context: Context):
    ...
    if collect_evaluate_results_call_tracker >= num_rounds:
        process_evaluate_results(results=evaluate_results, out_dir=context.run_config["out-dir"])

def process_fit_results(results: dict[str, list[float]], out_dir: str):
    base_path = os.path.join(out_dir, "performance")
    os.makedirs(base_path, exist_ok=True)
...
def collect_fit_results(metrics_dict: dict[str, float], context: Context):
    ...
    if collect_fit_results_call_tracker >= num_rounds:
        process_fit_results(results=fit_results, out_dir=context.run_config["out-dir"])
...
class FedAvgWrapper(FedAvg):
    def __init__(self, server_rounds, out_dir, **kwargs):
    ...
    def aggregate_fit(
        ...
        if server_round == self.server_rounds:
            base_path = os.path.join(self.out_dir, "latency")
            os.makedirs(base_path, exist_ok=True)
...
class WeightedFedAvg(FedAvg):
    def __init__(self, server_rounds, out_dir, fit_metrics_aggregation_fn: Optional[MetricsAggregationFn] = None,
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        n_samples = reduced_update_matrix.shape[0]
        n_features = reduced_update_matrix.shape[1]
        # Lower bound for MCD support_fraction according to sklearn docs
        min_support = (n_samples + n_features + 1) / (2 * n_samples)
        support_frac = max(min_support, min(0.9, len(results) / 10))
        support_frac = min(support_frac, 1.0) # Cap at 1.0
        ...
        if len(results) < 2:
            # Fallback for single-client case to avoid MinCovDet crash
            mcd_mean = reduced_update_matrix[0]
            mcd_inv_cov = np.eye(reduced_update_matrix.shape[1])
        else:
    ...
    def robustness_metrics_to_csvs(self):
        # Round
        base_path = os.path.join(self.out_dir, "robustness", "per_round")
        os.makedirs(base_path, exist_ok=True)
        ...
        # Global
        base_path = os.path.join(self.out_dir, "robustness", "overall")
        os.makedirs(base_path, exist_ok=True)
...
def server_fn(context: Context):
    ...
    out_dir = context.run_config["out-dir"]
